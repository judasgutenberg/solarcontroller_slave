...
byte IRhigh;
byte ir_packet[18]; 
int integers[12]={2295,34935,18615,51255,10455,43095,26775,59415,6375,39015,22695,13005};//last two are colon and dash
//old int integers[12]={255,32895,16575,49215,8415,41055,24735,57375,4335,36975,31371343,31357063};//last two are colon and dash
//commands are in this order: ??, up, down, left, right, enter(ok), delete, quit
//old: int commands[8]={0,49725,8925,17085,33405,41565,39015,31347373};
int commands[8]={32385,765,33405,57375,24735,8925,55845,3315};
bool menumode=false;
...
{ 
	...
	if (irrecv.decode(&results)  && canmenu) 
	{
		...
		//Serial.print((unsigned int)IRresval);
		//Serial.print(":");
    //Serial.print((unsigned int)irval);
    //Serial.print(":");
		//Serial.print((char)irval);
    //Serial.println("");
		if(!menumode)
		{
...
{  
	...
	{
		#if COMMANDTOMASTERMODE == 0
		Wire.write(commandpacket,18); //doesn't work when commandpacket is char, only byte!!
		#endif
		ClearCommandPacket();
		nonlcdmode=false;
		return;
